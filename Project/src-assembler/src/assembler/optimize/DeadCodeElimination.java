...
/**
 * If an instruction follows a return or an unconditional branch, and if the
 * instruction has no targeted labels, then that instruction is unreachable
...
public class DeadCodeElimination implements Optimization
  {
    ...
    public void optimize(List<Instruction> instructions, int instNum)
      {
        ...
        // When instNum = 1, create a map that maps labels -> n, where n is
        // the number of branch or call instructions that branch to that label.
        // If a branch instruction gets removed, decrement the corresponding n
        // for its target label.  Any instruction that follows a return or an
        // unconditional branch is unreachable if it has no labels OR if the
        // total count of branches to it is zero.
        if (instNum == 1)
          {
        ...
        if (symbol0 == Symbol.BR || isReturnSymbol(symbol0))
          {
            ...
            var labels1 = instruction1.labels();
    ...
    private String getLabelTarget(InstructionOneArg instruction)
      {
        ...
        Token targetLabel = instruction.arg();
    ...
    private boolean isBranchOrCallInstruction(Instruction instruction)
      {
        return isBranchOrCallSymbol(instruction.opcode().symbol());
      }
