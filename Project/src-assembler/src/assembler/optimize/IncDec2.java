...
import assembler.ast.InstructionDEC;
import assembler.ast.InstructionINC;
    ...
    public void optimize(List<Instruction> instructions, int instNum)
      {
        ...
        Symbol symbol0 = instruction0.opcode().symbol();
        Symbol symbol1 = instruction1.opcode().symbol();
        Symbol symbol2 = instruction2.opcode().symbol();
        ...
        var arg0  = inst0.arg().text();

        if (arg0.equals("1"))
          {
            Symbol symbol3 = instruction3.opcode().symbol();

            if (symbol3 == Symbol.ADD)
              {
                // replace ADD with INC
                var incToken = new Token(Symbol.INC);
                var labels   = instruction3.labels();
            ...
            inst1Labels.addAll(inst0.labels());
