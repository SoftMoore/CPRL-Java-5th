    ...
    public void optimize(List<Instruction> instructions, int instNum)
      {
        ...
        var symbol2 = instruction2.opcode().symbol();
        ...
        // make sure that instruction1 and instruction2 do not have any labels
        if (inst1.labels().isEmpty() && instruction2.labels().isEmpty())
          {
            switch (symbol2)
              {
                case ADD ->
                  {
                    ...
                    inst0.arg().setText(Integer.toString(sum));
                  }
                case SUB ->
                  {
                    int difference = value0 - value1;
                    inst0.arg().setText(Integer.toString(difference));
                  }
                case MUL ->
                  {
                    int product = value0*value1;
                    inst0.arg().setText(Integer.toString(product));
                  }
                case DIV ->
                  {
                    int quotient = value0/value1;
                    inst0.arg().setText(Integer.toString(quotient));
                  }
                case MOD ->
                  {
                    int remainder = value0%value1;
                    inst0.arg().setText(Integer.toString(remainder));
                  }
                default ->
                  {
                    return;
