...
import assembler.ast.AST;
import assembler.ast.Instruction;
import assembler.ast.Program;
        ...
        try (scanner)
          {
            ...
            if (!errorHandler.errorsExist())
              {
                printProgressMessage("...checking labels");
                program.checkLabels();
              }
            ...
            if (DEBUG)   // Java complains if I use DEBUG && optimize
              {
                if (optimize)
                  {
                    printProgressMessage("...program after performing optimizations");
                    printInstructions(program.instructions());
                  }
              }
            ...
            if (!errorHandler.errorsExist())
              {
                ...
                var outputStream = getTargetOutputStream(sourceFile);
                AST.setOutputStream(outputStream);
