...
// assume that all lines read from stdin contain less than 1000 characters
...
#endif
    // For Windows Command Prompt (or Windows Terminal), make sure that the console
    // font can display Unicode.  Truetype fonts such as Consolas and Lucida Console
    // will work.  Also, enter the command "chcp 65001" to change the code page
    // before running a program that uses non-ascii characters.
 ...
 */
void copyStringToHeap(wchar_t* s)
  {
    // allocate space on the heap for the sequence of characters
    int slength = wcslen(s);
    heapAlloc(BYTES_PER_CHAR*slength);
    int hpTemp = hp;
    for (int i = 0; i < slength; ++i)
      {
        putCharToAddr(s[i], hpTemp);
        hpTemp = hpTemp + BYTES_PER_CHAR;
      }

    // allocate space on the heap for length
    heapAlloc(BYTES_PER_INTEGER);
    putIntToAddr(slength, hp);
  }

/**
 * Copy sequence of characters from the specified string address to the heap.
 */
void copyStringToHeapAddr(int strAddress)
  {
    int strLength = getIntAtAddr(strAddress);

    // allocate space on the heap for the sequence of characters
    heapAlloc(BYTES_PER_CHAR*strLength);

    // skip over length
    int firstCharAddress = strAddress + BYTES_PER_INTEGER;

    for (int i = 0; i < strLength; ++i)
      {
        int sourceAddress = firstCharAddress + i*BYTES_PER_CHAR;
        wchar_t ch = getCharAtAddr(sourceAddress);
        int destAddress = hp + i*BYTES_PER_CHAR;
...
void getByte()
  {
    ...
    // Read 1 element of 1 byte from stdin
    size_t bytesRead = fread(&byteValue, 1, 1, stdin);
...
void getInt()
  {
    ...
    if (ferror(stdin))
         error(L"Invalid input");
    else if (result == WEOF || feof(stdin))
         eofFlag = 1;
...
void getString()
  {
    ...
    wchar_t* data = (wchar_t*) malloc(MAX_LINE_LENGTH*BYTES_PER_CHAR);
    int length = getln(data);
...
void loadConstStr()
  {
    ...
    pc = pc + BYTES_PER_INTEGER + length*BYTES_PER_CHAR;
  }
...
void putString()
  {
    ...
    // skip over length
    address = address + BYTES_PER_INTEGER;

    for (int i = 0; i < length; ++i)
      {
        putwchar(getCharAtAddr(address));
        address = address + BYTES_PER_CHAR;
      }
    fflush(stdout);
  }
...
void cvmstrcat()
  {
    ...
    int addressOfLeftOperand  = popInt();
    int addressOfRightOperand = popInt();
    int strLengthLeft  = getIntAtAddr(addressOfLeftOperand);
    int strLengthRight = getIntAtAddr(addressOfRightOperand);
    int strLengthConcatenated = strLengthLeft + strLengthRight;

    copyStringToHeapAddr(addressOfLeftOperand);
    copyStringToHeapAddr(addressOfRightOperand);
...
void printRegisters()
  {
    printf("Registers: PC=%d, BP=%d, SB=%d, SP=%d, HP=%d\n", pc, bp, sb, sp, hp);
  }
    ...
    if (hp < MEMORY_SIZE)
      {
        printf(".............................................. heap\n");
        printf("HP -> %4d: %d\n", hp, memory[hp]);
        for (int memAddr2 = hp + 1; memAddr2 < MEMORY_SIZE; ++memAddr2)
            printf("      %4d: %d\n", memAddr2, memory[memAddr2]);
      }
  }
